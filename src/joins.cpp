             ...
             bool bounded_result, uint64_t UPPER_BOUND, vector<rank_bv_64>& temp_bv)
{
    ...
    if (cur_level == max_level)
    {
        for (i = 0; i < nQ && children; ++i)
        {
            if (nAtt == 3) {
                ...
                throw std::runtime_error("SemiAND only supports nAtt = 3, 4, or 5");
            }
        }
    ...
    else
    {
        ...
        for (i = 0; i < children_to_recurse_size; ++i)
        {
            ...
            for (uint64_t j = 0; j < nQ; j++)
            {
                uint16_t mapped = Q[j]->getM(child);

                if (mapped >= k_d[j]) {
                    cerr << "[SemiAND] bad M mapping: qdag=" << j
                         << " child=" << child
                         << " mapped=" << mapped
                         << " k_d=" << k_d[j]
                         << endl;

                    throw std::runtime_error("SemiAND bad M mapping");
                }

                if (rank_vector[j][mapped] == 0) {
                    cerr << "[SemiAND] missing rank: qdag=" << j
                         << " child=" << child
                         << " mapped=" << mapped
                         << " cur_level=" << cur_level
                         << " root=" << roots[j]
                    ...
                    throw std::runtime_error("SemiAND missing rank");
                }

                root_temp[j] = k_d[j] * (rank_vector[j][mapped] - 1);
            }
            ...
            if (SemiAND(Q, root_temp, nQ, cur_level + 1, max_level,
...
void semiJoin(vector<qdag> &Q, bool bounded_result, uint64_t UPPER_BOUND)
{
    ...
    cout << "[semiJoin] level=" << level
         << " Q0_bv_u=" << Q[0].Q->bv[level].size()
         << " temp_u=" << temp[level].size()
         << " Q0_ones=" << Q[0].Q->bv[level].n_ones()
         << endl;
    }
